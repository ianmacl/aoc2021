...
def find_score(board, picked_numbers):
    sum = 0
    for i in range(0, 5):
        for j in range(0, 5):
    ...
    return sum * int(picked_numbers[-1])
...
for i in range(1, len(numbers)):
    for j in range(0, len(boards)):
        if j not in winning_boards:
            winning_board = board_won(boards[j], numbers[0:i])
            if winning_board is not None:
                winning_score = find_score(winning_board, numbers[0:i])
